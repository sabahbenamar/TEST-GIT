function Dde_licence(Chemin_licence,user);

DataLicence.CarteMere   = wmicGet('Win32_BaseBoard', 'SerialNumber');
DataLicence.Hostname    = wmicGet('cpu','SystemName');
DataLicence.Bios        = wmicGet('bios','SerialNumber');
DataLicence.DisqueDur   = wmicGet('Win32_DiskDrive','SerialNumber');

Excel = actxserver('Excel.Application');
Excel.Visible = false;
DataLicence.UserName    = Excel.UserName;
Quit(Excel);
user=DataLicence.UserName;

fid=fopen([Chemin_licence '\Data_licence_' user '.txt'],'w');
fprintf(fid,'%s %s %s %s %s ',DataLicence.Bios, DataLicence.CarteMere ,DataLicence.DisqueDur ,DataLicence.Hostname ,DataLicence.UserName);
fclose(fid);
